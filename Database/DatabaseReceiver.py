...
class DatabaseReceiver(DatabaseConnection):
    ...
    def get_course_info(self):
        ...
        data = self.select("*", self.course_table)
    ...
    def get_prereqs(self):
        ...
        data = self.select("*", self.prereqs_table)
        df = pd.DataFrame(data, columns=columns)
        df.set_index("index", inplace=True)
        return df
    ...
    def get_coreqs(self):
        ...
        data = self.select("*", self.coreqs_table)
        df = pd.DataFrame(data, columns=columns)
        df.set_index("index", inplace=True)
        return df

    def get_antireqs(self):
        """
        ...
        data = self.select("*", self.antireqs_table)
        df = pd.DataFrame(data, columns=columns)
        df.set_index("index", inplace=True)
        return df
